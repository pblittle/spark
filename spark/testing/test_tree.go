...
func CreateNewTree(config *wallet.TestWalletConfig, faucet *Faucet, privKey keys.Private, amountSats int64) (*pb.TreeNode, error) {
	coin, err := faucet.Fund()
	if err != nil {
		return nil, fmt.Errorf("failed to fund faucet: %w", err)
	}

	conn, err := DangerousNewGRPCConnectionWithoutVerifyTLS(config.CoordinatorAddress(), nil)
	if err != nil {
...
func CreateNewTreeWithLevels(config *wallet.TestWalletConfig, faucet *Faucet, privKey keys.Private, amountSats int64, levels uint32) (*wallet.DepositAddressTree, []*pb.TreeNode, error) {
	coin, err := faucet.Fund()
	if err != nil {
	...
	tree, err := wallet.GenerateDepositAddressesForTree(ctx, config, depositTx, nil, uint32(vout), privKey, levels)
	if err != nil {
