...
func CommitTransactionCoordinated(
	...
	if err != nil {
		return nil, fmt.Errorf("failed to authenticate with operator %s: %w", config.CoodinatorIdentifier, err)
	}
...
func FreezeTokensV1(
	...
	issuerPublicKey SerializedPublicKey,
...
func ExchangeRevocationSecretsManually(
	...
	conn, err := exchangeParams.TargetOperator.NewGRPCConnection()
	if err != nil {
