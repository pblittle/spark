...
func TestExecuteTaskWithAllOperators(t *testing.T) {
	...
	results, err := helper.ExecuteTaskWithAllOperators(context.Background(), config, &selection, task)
	require.NoError(t, err)
...
func TestExecuteTaskWithAllOperators_Error(t *testing.T) {
	...
	_, err = helper.ExecuteTaskWithAllOperators(context.Background(), config, &selection, task)
	require.Error(t, err)
