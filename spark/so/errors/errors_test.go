...
func TestErrorInterceptor_NoError_ReturnsValue(t *testing.T) {
	...
	got, err := ErrorMaskingInterceptor(true)(t.Context(), nil, serverInfo, okHandler)
...
func TestInternalErrorDetailMasking(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantDetails:    false,
			handler: func(_ context.Context, _ any) (any, error) {
				return nil, FailedPreconditionErrorf("utxo not found: txid: %s vout: %d", "492aef9399b186196effc64f5a754fb6ccff5942adefde9786da5249eb9dde47", 0)
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := ErrorMaskingInterceptor(tt.detailedErrors)(t.Context(), nil, serverInfo, tt.handler)
			require.Error(t, err)
