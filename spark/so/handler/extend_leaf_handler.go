...
func (h *ExtendLeafHandler) extendLeaf(ctx context.Context, req *pb.ExtendLeafRequest, requireDirectTx bool) (*pb.ExtendLeafResponse, error) {
	...
	if directNodeSigningJob != nil && directRefundSigningJob != nil && directFromCpfpRefundSigningJob != nil {
		...
		if uint64(newDirectNodeTx.TxOut[0].Value) >= leaf.Value {
			return nil, fmt.Errorf("new direct node tx output value must be less than leaf value, leaf value: %d, direct node tx value: %d", leaf.Value, newDirectNodeTx.TxOut[0].Value)
		}
		...
		if uint64(directRefundTx.TxOut[0].Value) >= leaf.Value {
			return nil, fmt.Errorf("direct refund tx output value must be less than leaf value, leaf value %d, direct refund tx value: %d", leaf.Value, cpfpRefundTx.TxOut[0].Value)
		}
		if uint64(directFromCpfpRefundTx.TxOut[0].Value) >= leaf.Value {
			return nil, fmt.Errorf("direct from cpfp refund tx output value must be less than leaf value, leaf value %d, direct from cpfp refund tx value: %d", leaf.Value, directFromCpfpRefundTx.TxOut[0].Value)
		}
