...
func (h *LightningHandler) ValidateGetPreimageRequest(
	...
	for i := range directTransactions {
		...
		if len(directTx.TxOut) <= 0 {
			return fmt.Errorf("direct tx vout out of bounds")
		}
	...
	for i := range directFromCpfpTransactions {
		...
		if len(cpfpTx.TxOut) <= 0 {
			return fmt.Errorf("direct from cpfp vout out of bounds")
		}
	...
	for i := range directFromCpfpTransactions {
		...
		if len(directFromCpfpRefundTx.TxOut) <= 0 {
			return fmt.Errorf("direct from cpfp tx vout out of bounds")
		}
...
func (h *LightningHandler) GetPreimageShare(ctx context.Context, req *pb.InitiatePreimageSwapRequest) ([]byte, error) {
	...
	_, err = h.storeUserSignedTransactions(ctx, req.PaymentHash, preimageShare, req.Transfer.LeavesToSend, req.Transfer.DirectLeavesToSend, req.Transfer.DirectFromCpfpLeavesToSend, transfer, status, req.ReceiverIdentityPublicKey)
	if err != nil {
