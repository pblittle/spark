...
func ValidateOwnershipSignature(signature []byte, hash []byte, issuerOrOwnerPublicKey []byte) error {
	if signature == nil {
		return fmt.Errorf("ownership signature cannot be nil")
	}
	if hash == nil {
		return fmt.Errorf("hash to verify cannot be nil")
	}
	if issuerOrOwnerPublicKey == nil {
		return fmt.Errorf("owner public key cannot be nil")
	}

	pubKey, err := secp256k1.ParsePubKey(issuerOrOwnerPublicKey)
	if err != nil {
