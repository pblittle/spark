...
func calculateCurrentSupplyByTokenIdentifier(ctx context.Context, tokenIdentifier []byte) (*big.Int, error) {
	return calculateCurrentSupply(ctx, func(q *ent.TokenMintQuery) *ent.TokenMintQuery {
		return q.Where(tokenmint.TokenIdentifierEQ(tokenIdentifier))
	})
}
...
func calculateCurrentSupplyByIssuerKey(ctx context.Context, issuerPublicKey []byte) (*big.Int, error) {
	return calculateCurrentSupply(ctx, func(q *ent.TokenMintQuery) *ent.TokenMintQuery {
		return q.Where(tokenmint.IssuerPublicKeyEQ(issuerPublicKey))
	})
}

// calculateCurrentSupply is a helper function that executes the common query logic.
func calculateCurrentSupply(ctx context.Context, whereClause func(*ent.TokenMintQuery) *ent.TokenMintQuery) (*big.Int, error) {
	db, err := ent.GetDbFromContext(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to get or create current tx for request: %w", err)
	}

	signedMints, err := whereClause(db.TokenMint.Query()).
		Where(tokenmint.HasTokenTransactionWith(tokentransaction.StatusEQ(st.TokenTransactionStatusSigned))).
		WithTokenTransaction(func(q *ent.TokenTransactionQuery) {
			q.WithCreatedOutput().
				Where(tokentransaction.StatusEQ(st.TokenTransactionStatusSigned))
		}).
		All(ctx)
	if err != nil {
	...
	return calculateTotalMintedFromMints(signedMints), nil
}

// calculateTotalMintedFromMints sums up the total minted amount from a list of token mints.
func calculateTotalMintedFromMints(signedMints []*ent.TokenMint) *big.Int {
