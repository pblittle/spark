...
func TestReceiveLightningPayment(t *testing.T) {
	...
	invoice, _, err := wallet.CreateLightningInvoiceWithPreimage(context.Background(), userConfig, fakeInvoiceCreator, amountSats, "test", preimage)
	require.NoError(t, err)
...
func TestReceiveLightningPaymentV2(t *testing.T) {
	// Create user and ssp configs
	userConfig, err := testutil.TestWalletConfig()
	require.NoError(t, err)
	...
	authToken, err := wallet.AuthenticateWithServer(context.Background(), sspConfig)
	...
	transfer, err := wallet.DeliverTransferPackage(tmpCtx, sspConfig, response.Transfer, sparkClient, leaves, nil)
	require.NoError(t, err)
...
func TestSendLightningPaymentV2(t *testing.T) {
	...
	// Create gRPC client for V2 function
	conn, err := common.NewGRPCConnectionWithTestTLS(userConfig.CoodinatorAddress(), nil)
	require.NoError(t, err, "failed to create grpc connection")
	defer conn.Close()

	authToken, err := wallet.AuthenticateWithServer(context.Background(), userConfig)
	require.NoError(t, err, "failed to authenticate sender")
	tmpCtx := wallet.ContextWithToken(context.Background(), authToken)

	sparkClient := pb.NewSparkServiceClient(conn)

	transfer, err := wallet.DeliverTransferPackage(tmpCtx, userConfig, response.Transfer, sparkClient, leaves, nil)
	require.NoError(t, err)
	assert.Equal(t, spark.TransferStatus_TRANSFER_STATUS_SENDER_KEY_TWEAK_PENDING, transfer.Status)
...
func TestReceiveLightningPaymentWithWrongPreimage(t *testing.T) {
	...
	invoice, _, err := wallet.CreateLightningInvoiceWithPreimageAndHash(context.Background(), userConfig, fakeInvoiceCreator, amountSats, "test", preimage, wrongPaymentHash)
	require.NoError(t, err)
	assert.NotNil(t, invoice)
